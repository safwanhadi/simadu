...
def create_jenis_layanan(apps, schema_editor):
    ...
    matches = list(JenisLayanan.objects.filter(url='yanpangkat').order_by('pk'))
    layanan = matches[0] if matches else JenisLayanan(url='yanpangkat')
    ...
    layanan.nama = 'Kenaikan Pangkat'
    layanan.status = True
    layanan.icon = 'fa-level-up-alt'
    layanan.save()


def delete_jenis_layanan(apps, schema_editor):
    JenisLayanan = apps.get_model('layanan', 'JenisLayanan')
    JenisLayanan.objects.filter(url='yanpangkat').update(status=False)


class Migration(migrations.Migration):
    dependencies = [
        ('layanan', '0027_complete_layanan_naik_pangkat'),
    ]
